(function(){
    $(function() {
        ...
        function initInformationComment(){
            $container = $("#main-box");
            $.ajax({
                url: basePath+"/data/information-comment.html",
                async: false,
                contentType: "application/x-www-form-urlencoded; charset=UTF-8",
                success: function (data) {
                    $container.html(data);
        ...
        function initTable1(title,sendAuthor,dateAT,date) {
            ...
            function operateFormatter(value, row){
                return '<a href="#" class="edit" data-id="' + row.INID + '"><i class="glyphicon glyphicon-pencil both-5"></i></a>' +
                    '<a href="#" class="comment" data-id="' + row.INID + '"><i class="glyphicon glyphicon-comment both-5"></i></a>';
            }
            //查询草稿箱数量
            function cgxcount(){
                $.ajax({
